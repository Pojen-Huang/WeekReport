copy ..\LogHome\E7-NPDC-DP-02.txt .\RAWDATA
#Get Hostname from log file
[string]$hostname =  sed -n '15p' RAWDATA | sed  's/#//'
echo "1 - {Hostname}  = $hostname "
#Get UPtime
[string]$uptime = cat RAWDATA | grep 'Up Time:' | awk '{print $3,$4,$5,$6,$7,$8,$9,$10}'
echo "2 - {Uptime} = $uptime"
#Get macaddr number
[string]$macaddr = cat RAWDATA | grep 'Base MAC:' | awk '{print $3}'
echo "3 - {MACAddr} =  $macaddr"
#Get CPU Usage
[string]$cpu = cat RAWDATA | grep 'Master Utilization' | awk '{print $4}'
echo "4 - {CPU} = $cpu"
#Get CPU Temperture
[int]$Temp1= cat RAWDATA | grep -A4 'CPU Index' | sed -n '3p' | awk '{print $2}'
[int]$Temp2= cat RAWDATA | grep -A4 'CPU Index' | sed -n '5p' | awk '{print $2}'
echo "5 - {Temp1} = $Temp1"
echo "6 - {Temp2} = $Temp2"
#Get memory used
...
echo "9 - {count} = $count"	
$filename1='Radware_DP-3016_2_(第'
$filename2='次).doc'
...
copy ..\Template\temp_DP.doc .\working.doc
