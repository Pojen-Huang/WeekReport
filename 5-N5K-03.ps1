copy ..\LogHome\E7-NPDC-N5596-03.txt .\RAWDATA
$sn=0
#Get Hostname from log file
[string]$hostname = sed -n '16p' RAWDATA | sed  's/#//'
$sn+=1 ; echo "$sn - {Hostname} = $hostname "
#Get UPtime
[string]$uptime = cat RAWDATA | grep 'Kernel uptime is' | awk '{print $4,$5,$6,$7,$8,$9,$10,$11}'
$sn+=1 ; echo "$sn - {Uptime} = $uptime"
#Get Serial number
[string]$serial =  cat RAWDATA | grep  "PID: N5K-C5596UP       , VID: V01 , SN: FOX" | awk '{print $8}'
$sn+=1 ; echo "$sn - {Serial} =  $serial"
#Get CPU Usage
[int]$cpu_user = cat RAWDATA | grep 'CPU states  :' | awk '{print $4}' | sed  's/%//'
[int]$cpu_kernel = cat RAWDATA | grep 'CPU states  :' | awk '{print $6}' | sed  's/%//'
[int]$cpu = $cpu_user + $cpu_kernel 
$sn+=1 ; echo "$sn - {CPU_Users} = $cpu_user"
$sn+=1 ; echo "$sn - {CPU_Kernel} = $cpu_kernel"
$sn+=1 ; echo "$sn - {CPU} = $cpu"
#Get memory used
[int]$MEM_used = cat RAWDATA | grep 'Memory usage:' | awk '{print $5}' | sed  's/K//'
$sn+=1 ; echo "$sn - {Memory_used} = $MEM_used"
#Get memory total
[int]$MEM_total= cat RAWDATA | grep 'Memory usage:' | awk '{print $3}' | sed  's/K//'
...
copy ..\Template\temp_N5596.doc .\working.doc 
...
$textToReplace = @{
	...
	"{redundantNode}" = "E7-NPDC-N5596-03與E7-NPDC-N5596-04互為備援"
}
